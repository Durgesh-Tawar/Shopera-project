...
async function fetchProducts() {
    try {
        ...
        if (typeof window.products !== 'undefined' && window.products.length > 0) {
            ...
            console.log('✅ Local Products Loaded:', products.length);
            handleURLParams(); // Use URL params for initial render
        } else {
...
function renderProducts(filter = "all", search = "", subcategory = "") {
    if (!productGrid) return;
    console.log(`🧐 Rendering Products (Filter: ${filter}, Search: ${search}, Subcat: ${subcategory})`);
...
/* LEGACY VOICE SEARCH REMOVED - NOW HANDLED BY voice-assistant.js
